...
def parse_joystick_data(data):
    try:
        ...
        if match:
            ...
            return angle, strength #각도, 힘 데이터 반환
    except:
        pass
    return None, None


def get_direction_from_angle(angle, strength): #각도 별 함수 호출 위한 문자열 반환 함수 
...
def main():
    ...
    try:    
        while True:
            if gData:
                angle, strength = parse_joystick_data(gData) #gdata 함수 호출
                direction = get_direction_from_angle(angle, strength) # 방향 함수 호출
